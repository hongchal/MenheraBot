  ...
  return (
    <div style={{ margin: "16px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 8 }}>
        <span style={{ fontFamily: "Pretendard, sans-serif", fontSize: 13, color: "#888" }}>집착 온도</span>
        <span style={{ fontFamily: "Pretendard, sans-serif", fontSize: 40, fontWeight: 700, color, lineHeight: 1 }}>{value}°</span>
      </div>
      <div style={{ position: "relative", height: 6, background: "#1a1a1a", borderRadius: 3 }}>
        <div style={{
          position: "absolute", left: 0, top: 0, height: "100%",
          width: `${value}%`, background: color, borderRadius: 3,
        }} />
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
        <span style={{ fontFamily: "Pretendard, sans-serif", fontSize: 10, color: "#555" }}>0°</span>
        <span style={{ fontFamily: "Pretendard, sans-serif", fontSize: 11, color, fontWeight: 600 }}>{label}</span>
        <span style={{ fontFamily: "Pretendard, sans-serif", fontSize: 10, color: "#555" }}>100°</span>
      </div>
    </div>
  );
}
...
export default function MenheraBot() {
    ...
    try {
      ...
      while (true) {
        ...
        if (!jsonParsed) {
          ...
          if (newlineIdx !== -1) {
            try {
              const parsed: Structured = JSON.parse(buffer.slice(0, newlineIdx).trim());
              setStructured(parsed);
              jsonParsed = true;
              const rest = buffer.slice(newlineIdx + 1);
              if (rest) setChatText(rest);
              buffer = "";
  ...
  const emotionColor = structured ? (emotionColors[structured.emotion_label] ?? "#888") : "#888";
    ...
    <div style={{
      ...
      <div style={{ padding: "48px 24px 32px", borderBottom: "0.5px solid #222" }}>
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: "#f0f0f0", margin: 0, lineHeight: 1.3 }}>
            그 사람 행동,<br />분석해보자
          </h1>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "32px 24px 0" }}>
        ...
        <button
          onClick={runJudge}
          disabled={loading || streaming || !situation.trim()}
          style={{
            width: "100%", padding: "14px",
            background: loading || streaming || !situation.trim() ? "#1a1a1a" : "#f0f0f0",
            border: "none", borderRadius: 8,
            cursor: loading || streaming || !situation.trim() ? "not-allowed" : "pointer",
            color: loading || streaming || !situation.trim() ? "#444" : "#0d0d0d",
            fontFamily: "Pretendard, sans-serif", fontSize: 14, fontWeight: 600,
            transition: "all 0.15s",
          }}
        >
          {loading ? "분석 중..." : streaming ? "말하는 중..." : "판단하기"}
        </button>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{
                    ...
                    <p style={{ fontSize: 14, color: "#ddd", lineHeight: 1.85, margin: 0, whiteSpace: "pre-wrap" }}>
                      {chatText}
                      {streaming && (
                        <span style={{
                          display: "inline-block", width: 2, height: 14,
                          background: emotionColor, marginLeft: 2, verticalAlign: "middle",
